        ...
        this.particles.forEach((particle, index) => {
            ...
            this.particles.slice(index + 1).forEach(other => {
                ...
                if (distance2 < 100) {
                    ...
                    this.ctx.strokeStyle = `rgba(59, 130, 246, ${0.2 * (1 - distance2 / 100)})`;
...
function addSkill() {
    ...
    if (portfolioData.skills.some(s => s.skill.toLowerCase() === skill.toLowerCase())) {
...
function displaySkill(skillObj) {
    ...
    skillTag.innerHTML = `
        ...
        <button class="skill-remove" onclick="removeSkill('${skillObj.skill.replace(/'/g, "\\'")}')" type="button">&times;</button>
    `;
...
function removeSkill(skillName) {
    portfolioData.skills = portfolioData.skills.filter(s => s.skill !== skillName);
...
function displayEducation(eduObj) {
    ...
    eduItem.innerHTML = `
        ...
        <button class="remove-btn" onclick="removeEducation('${eduObj.institution.replace(/'/g, "\\'")}', '${eduObj.degree.replace(/'/g, "\\'")}')">Remove</button>
    `;
...
function clearEducationForm() {
    ['edu-institution', 'edu-degree', 'edu-year', 'edu-description'].forEach(id => {
...
function displayProject(projectObj) {
    ...
    projectItem.innerHTML = `
        ...
        <div class="project-technologies">Technologies: ${projectObj.technologies}</div>
        ${projectObj.github || projectObj.demo ? `<div class="project-links">
            ${projectObj.github ? `<a href="${projectObj.github}" target="_blank">GitHub</a>` : ''}
            ${projectObj.demo ? `<a href="${projectObj.demo}" target="_blank">Demo</a>` : ''}
        </div>` : ''}
        <button class="remove-btn" onclick="removeProject('${projectObj.title.replace(/'/g, "\\'")}')">Remove</button>
    `;
...
function clearProjectForm() {
    ['project-title', 'project-description', 'project-technologies', 'project-github', 'project-demo', 'project-image'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.value = '';
    });
}
...
function removeProject(title) {
    portfolioData.projects = portfolioData.projects.filter(p => p.title !== title);
...
function showCodeTab(tab) {
    ...
    document.querySelectorAll('.code-tab-content').forEach(content => content.classList.add('hidden'));
...
async function downloadPortfolio() {
    ...
    if (typeof JSZip === 'undefined') {
        showToast('JSZip library not loaded. Please refresh and try again.', 'error');
        return;
    }
    ...
    try {
        ...
        // Add README
        const readme = `# Portfolio - ${portfolioData.personal.fullName || 'Generated Portfolio'}
...
`;
        ...
        const fileName = `portfolio-${portfolioData.selectedTemplate || 'modern'}-${Date.now()}.zip`;
...
function generatePortfolioCode() {
    console.log('🎨 Generating portfolio code for template:', portfolioData.selectedTemplate);
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="skill-card bg-gray-50 p-6 rounded-xl shadow-sm hover:shadow-md transition-all" data-index="${index}">
                    <div class="flex justify-between items-center mb-4">
                        <h3 class="font-semibold text-gray-900">${skill.skill}</h3>
                        <span class="text-xs px-3 py-1 bg-blue-100 text-blue-600 rounded-full">${skill.category}</span>
                    </div>
                    <div class="text-sm text-gray-600 mb-2">${skill.level}</div>
                    <div class="w-full bg-gray-200 rounded-full h-2">
                        <div class="skill-bar bg-blue-600 h-2 rounded-full" data-width="${getSkillPercentage(skill.level)}"></div>
                    </div>
                </div>
                `).join('')}
            </div>
                ...
                <div class="project-card bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all" data-index="${index}">
                    ...
                    <div class="p-6">
                        ...
                        <div class="flex gap-4">
                            ...
                            ${project.demo ? `<a href="${project.demo}" target="_blank" class="text-blue-600 hover:underline">Live Demo</a>` : ''}
                        </div>
                    </div>
                </div>
                `).join('')}
...
// Additional template generators (simplified for brevity)
function generateCreativeTemplate() {
    return generateModernTemplate(); // For now, using modern template
}
...
console.log('🎉 Portfolio Generator fully loaded with enhanced button handling!');